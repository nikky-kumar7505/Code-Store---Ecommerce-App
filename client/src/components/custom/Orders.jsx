...
const Orders = () => {
  ...
  return (
    <>
       <h1 className='text-3xl font-bold mb-2 ml-5' >Orders</h1>
       <div className="flex flex-col gap-5 mx-auto ">
        <div className="space-y-8">
          ...
          <div className="grid space-y-1 gap-2 sm:w-[80vw] ">
            {
              orders.length ===0 ? <h2 className='text-primary text-3xl'>Nothing To Show, Please add some products...</h2> : (
                orders.map((item) => (
                  <Card key={item._id} className="space-y-2 p-3 shadow-md  ">
                    ...
                    <div className="">
                      <p className='flex justify-between sm:justify-start gap-2 items-center px-3' >
                        ...
                        <span className='text-sm text-customGrey'>₹{item?.amount}</span>
                      </p>
                      <p className='flex justify-between sm:justify-start gap-2 items-center px-3' >
                      ...
                      <p className='flex justify-between sm:justify-start gap-2 items-center px-3' >
                        ...
                        <span className='text-sm text-customGrey'> {item?.userId?.name} </span>
                      </p>
                      <p className='flex justify-between sm:justify-start gap-2 items-center px-3' >
                        <span className='font-bold'>Email</span>
                        <span className='text-sm text-customGrey'> {item?.userId.email} </span>
                      </p>
                      <p className='flex justify-between sm:justify-start gap-2 items-center px-3' >
            ...
            <PaginationItem>
              {
                Array.from({length : totalPages }, (data, i) =>(
                  <PaginationLink href="#" onClick ={()=>setCurrentPage(i+1)} key={i} >
                    {i+1}
                  </PaginationLink>
                ))
              }
